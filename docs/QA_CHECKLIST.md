...
- [ ] PRO-31 **Play**: start, complete the objective, extract, see the results, and return to camp with the inventory kept.
  ...
  - The generator part is a real key item (`generator_part`, 1.2 kg, its own alternator model and icon). Searching the Ash Market room puts it in the pack and completes the Retrieve objective; with the pack too full the room stays unsearched and a toast says so. Back in camp, when the generator has broken (a camp event), Repair uses a carried part first, with no engineer, scrap or tape needed. Search the room with a full pack, drop something and search again, then bring the part home to a broken generator and check it runs again and the part is gone.
...
- [ ] PRO-35 **Play**: contact shadows ground the props, and Medium holds 60 fps. Compare with the tour screenshots.
  ...
  - The base grade is a saved URP profile, `Assets/Resources/PostFX/OutpostZero_PostFX.asset`, and the aim blur is `OutpostZero_AimDepth.asset`. The rig loads both and works on each volume's own clone, so the per-frame hurt, raid, poison, night and brightness layers never change the assets; with an asset missing it builds the same values in code. `PostFxProfileTests` holds each asset's components, URP scripts, on/off state and values equal to the rig's constants. Open the profile in the Inspector, nudge the bloom, enter Play and see it change; leave Play and check the asset kept your value, not a runtime one.
  - The Damage, Toxic and Sanctuary looks are their own volumes above the base (Toxic 22, Sanctuary 23, Damage 24), with profiles `OutpostZero_Damage`, `OutpostZero_Toxic` and `OutpostZero_Sanctuary` holding each look at full strength. The Damage weight is the hurt amount; Toxic and Sanctuary fade in and out over about 0.7 s with poison and camp. Raid, bolt and ash tints stay on the base volume. `PostFxProfileTests` checks that the stacked volumes give the same frame the single volume did. Check: in the Volume debugger, drop below half health and see only DamageVolume's weight rise; get poisoned and see ToxicVolume fade in; walk home and see SanctuaryVolume fade in. Camp and street share one yard, so the sanctuary volume is weighted by the camp state rather than bounded by a box.
- [ ] PRO-36 **Play**: an expedition runs from afternoon to night, lamps pool on wet asphalt, and the flashlight casts shadows. No light leaks through `Building_*` walls.
  ...
  - Shadows: the default Medium tier casts to 45 m with 2 cascades, as the issue asks; Low trims to 18 m and High and Ultra reach 60 and 80 m. Soft shadows are supported on Medium and up (before, every URP asset had them off, so the lights' Soft setting fell back to hard). The sun gets a 0.2 m near plane and a 0.6 normal bias. Check: a leader's shadow meets their feet with no gap.
- [ ] PRO-37 **Play**: rain plays and looks different from clear weather, and fog hides the map edge.
  ...
  - Height fog: both renderers carry a `FullScreenPassRendererFeature` named HeightFog before transparents, reading the depth texture and blending `OutpostZero/HeightFog` over the target. `WeatherController` pushes the thickness, rise and colour each frame through `HeightFog.Push`, so fog pools in the street and thins on the roofs, and thickens with bad weather and night. Check: in a fog day the road is veiled from about 6 m while a rooftop view stays clearer; leaving the expedition clears it. The mist-bank meshes still sit on top. `HeightFogTests` covers the maths and the renderer, material and shader wiring.
...
- [ ] PRO-58 **Play**: B places walls, a generator with lights, a farm and a purifier. Each changes the daily numbers and raid behaviour.
  ...
  - Finished modules wear the baked base-kit prefabs (`Resources/ModuleLooks.asset`): the wood-and-wire barricade, cot, water collector (also used for the purifier), watchtower, diesel generator, workbench, campfire cooker, crates, street lamp, oil barrel and spikes, plus a hydroponic farm (two pallet beds of seedlings under a tarp hoop) and an auto-turret (a tripod gun with a sensor, an ammo can and a battery). Build sites keep the plywood scaffold box. A battered module sits lower and darker. The farm and turret also have codex pages with their mesh renders. Build a farm and a turret, and check both show the new models once finished.
  ...
  - Perimeter integrity (`Perimeter`): each raid approach scores 0 to 100 from the integrity of the finished barricades within its reach (two whole barricades fill a side), and the yard score is the four averaged. The camp board and the raid's opening line show it ("Perimeter 50%"). An open yard draws two more raiders, a full wall two fewer, and a covered side takes up to two off each strike. Wall the gate and alley only, start a raid, and check the line reads 50% and the chewed barricades lower the score.
  - Automated: `BuildGhostTests` (verdicts, bills, refunds and lines) and `ModuleBookTests`, `PerimeterTests`.
...
- [ ] PRO-62 **Play**: scrap buys from the merchant, caravan visits can be planned around, and reputation changes prices and unlocks stock.
  ...
  - A faction asset with a new id (lower-case letters, digits and underscores) adds a faction after the built-in four: it trades from its own table, joins the visit calendar and is named at the gate by its label. Saves keep standing as `id=value` entries, so the list can grow: a save from a smaller book gives new factions 0, and a save that meets a book missing one of its factions keeps that entry and writes it back, so the standing returns with the faction. Quests and the militia ambush stay with the built-in ids. `FactionBookTests` covers the roster, bad ids, the calendar, and the save round trip both ways. Add a test faction asset, run Sync Faction Book, and wait in camp until it visits; save, remove the asset, load and save, restore it, and load again to see its standing come back.
...
- [ ] PRO-64 **Automated** (round trip, `.bak` recovery, v1 fixture migration) and **Play**: quit at each camp phase and Continue restores it. On permadeath, try Save from the pause menu on the street: the button reads "Save (camp only)", nothing is written and a toast says why; the same button works when paused in camp. On Merciful, save from the pause menu mid-street, walk on and take hits, then load that slot: the same street reopens without spending travel, with the clock, kills, scrap, objective progress, the leader's spot, facing and health, and the live zombies' types, spots and health as they were. Street loot and searched rooms are rebuilt fresh. Automated: `StreetSnapshotTests`. Automated: `SaveRegistryTests` (id order, waiting parts, duplicate ids, v2 codex migration).
  ...
  - The leader's kit is saved: the pack (items, scrap and medkits), the four belt pockets, and each weapon slot in order with its magazine and reserve, as the `leader_kit` part. Before, a camp save kept none of it, so Continue handed back whatever the scene started with. Check: in camp, carry 3 bandages and a molotov on the belt, pick up the shotgun on a street and fire two shells, go home and save; start a new game, then load the slot and see the same pack, pockets, shotgun slot and rounds. A save from before this has no kit part and leaves the leader as they are. Automated: `LeaderKitTests`.
...
- [ ] PRO-68 **Automated** (same seed gives the same street, blocks stay walkable) and **Play**: a campaign win takes about 6 to 8 hours.
  ...
  - Smaller multi-storey buildings (one cell, or two cells in a row) are too narrow for a flight, so each storey but the top gets a steel ladder (`Kit_ladder`, a baked kit piece) against the wall across from the door. **Climb the ladder** at its foot steps you onto the floor above, beside the ladder's top, and **Climb down** there brings you back to the foot; the slab stays whole, as if through a hatch. Both spots keep the 0.4 m player capsule clear of the walls and the ladder even in a one-cell room, and props stay off the ladder and both spots. A following survivor comes along. Climb to the top floor of a one-cell storefront and a two-cell apartment, with the door on each side.
  - Automated: `GameSystemsTests` rebuilds every district for seeds 1 to 100, checks the same seed gives the same street, and checks that every block stays walkable from the spawn to the point of interest and the way out. `RoadLotTests` checks that the blocks cover every lot exactly once and include every shape; that every building uses known kit pieces, is the same for the same seed, and varies in height (1 to 3 storeys) and door (single, double, garage); that each storey has its floor and a closed outline; that the door faces the road; that each building stays 0.75 m inside its block and clear of the crate; that props stand inside, clear of the door; and that every upper storey of a 2 by 2 building has a flight inside its walls, with 1.9 m head room, a top step at the next floor, a landing tile and no props on it, for all four door sides; and that every upper storey of a smaller building has a ladder against the back wall, at least 1 m from the door, with both climb spots on a floor tile, 0.45 m clear of the walls and ladders, and no props on them, for all four door sides.
