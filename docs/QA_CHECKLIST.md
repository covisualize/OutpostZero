...
- [ ] PRO-67 **Automated** (`TutorialFlowTests`: the Day 1 camp track (assign a task, read the stores, place a barricade, craft a bandage, launch) and the expedition track advance in order; every gate is emitted somewhere; the camp track is remembered in the codex; prompts show pad buttons while a pad is in use; the first expedition is a quiet street with three scripted walkers ahead of the start and no waves; the crouch, reload and flashlight hints arrive when a zombie is near, the magazine runs dry and you step into a dark room) and **Play**: 3 fresh testers finish Day 1 and the first expedition with no README.
