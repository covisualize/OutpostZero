...
- [ ] PRO-69 **Automated** (`VisionModeTests`: the setting cycles off, blue-yellow, red-teal and mono and saves with the enemy outline; zombie eyes and rims take the palette colour, and the outline hardens the rim) and **Play**: the game is playable in each colour-blind mode (off, blue-yellow for deuteranopia and protanopia, red-teal for tritanopia, mono), with the enemy outline on, and with sound off using captions. Switching EN/ES re-renders every screen.
