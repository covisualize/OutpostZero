...
- [ ] PRO-32 **Automated** (`AimRigTests`: aim weight per stance, the twist limit on the correct side, socket drift cap, reload clip speed and its late-only finish event, and the controller asset's IK pass and `ReloadSpeed` wiring) and **Play**: every state animates, the reload clip ends as the rounds go in (also when wounded), and feet don't slide at walk speed. The chest and head turn toward the cursor up to 70 degrees ahead of the hips, and the gun moves with the right hand.
