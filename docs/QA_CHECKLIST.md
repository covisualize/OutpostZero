...
- [ ] PRO-58 **Play**: B places walls, a generator with lights, a farm and a purifier. Each changes the daily numbers and raid behaviour.
  ...
  - Modules are `BuildingModuleDefinition` assets in `Assets/Data/Modules` (16), listed in `Resources/ModuleBook.asset`. Each holds its scrap cost, builder hours, stand-in size, whether it shrinks as it wears, surface and tint. The yard loads the book on start, and the code tables answer only for kinds the book lacks. **Tools > Outpost Zero > Sync Module Book** (also run by Generate Default Data) creates missing assets without touching tuned ones. `ModuleBookTests` holds every asset equal to the built-in table and checks that a loaded book overrides cost, hours and look.
  - Deviation: behaviour (what a wall stops, what a generator powers) stays in code keyed by `ModuleKind`, every module takes one cell, and costs are scrap only.
