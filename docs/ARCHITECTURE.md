...
| `OutpostZero.Items` | `Items/` | `ItemCatalog`, loot tables, `LootContainer` |
...
flowchart LR
    ...
    Code[ItemCatalog, CampaignBoard, Loc tables] --> Runtime
```

Items, districts, recipes and strings are defined in code (`ItemCatalog`, `CampaignBoard`, `Localization`), so saves and tests don't depend on imported assets. Weapons and zombies come from `WeaponDefinition` and `ZombieArchetype` assets under `Assets/Data`, generated by `DefaultDataGenerator`.
