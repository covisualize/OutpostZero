...
- Characters (`/Characters/`) get a Generic rig with an avatar created from the model, and uncompressed animation. Their clips are handed to `SurvivorAnimatorBuilder.AssignMotions`. The leader's clips go into `Resources/SurvivorLocomotion.controller`, and every other character gets its own copy of the graph at `Resources/Animators/<model>.controller` (`CharacterRig`). Alternate takes (`IdleB`, `WalkB`, `Shamble`, `DeathB`, `DeathC`) become 1D blend trees on `IdleVariant`, `GaitVariant` and `DeathVariant`. The reload is a blend of `Reload` (pistol and SMG), `ReloadShotgun` and `ReloadRifle` on `ReloadVariant`, which the body sets from the gun in hand. A model with a `Dissolve` take gets a `Dissolve` state after `Death`, so a zombie's corpse sinks into a heap while it melts. Walk, sprint, crouch-walk and dash play at `MoveRate`, so the stride matches the body's speed. Attack and Reload play on an `UpperBody` override layer whose avatar mask, cut from the model's own bone paths, covers the Spine subtree, so the legs keep their gait during a swing or reload. Other models import without animation.
- The rig is Generic rather than Humanoid on purpose: the upper-body mask and the aim rig address bones by path and name, which Humanoid retargeting would remap. The skeleton already has every Humanoid-required bone (`character_rig.REQUIRED_BONES`), so a later switch needs only the importer change and a mask rebuild.
- Every mesh that the manifest unwraps (`bake.uv`, which is on by default) must have non-overlapping UVs. `fbx_uv.py` measures the exported layout, and both `pipeline.py` and `asset_audit.py` fail when more than 2% of the covered texels are shared.
